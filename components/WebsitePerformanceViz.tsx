...
const NothingFoundScreen: React.FC = () => (
    <motion.div
        ...
        exit={{ opacity: 0 }}
    >
        <p className="text-sm sm:text-base text-stone-500 dark:text-stone-400">SCANNING FOR CONVERSIONS...</p>
...
const ConversionsRecordingScreen: React.FC = () => (
    <motion.div
        ...
        exit={{ opacity: 0 }}
    >
        <p className="text-sm sm:text-base text-stone-500 dark:text-stone-400">ANALYZING FUNNEL...</p>
...
const WebsitePerformanceViz: React.FC = () => {
    ...
    return (
        <motion.div 
            ...
            className="relative w-full max-w-4xl mx-auto aspect-[1.6/1] sm:aspect-[2.2/1] bg-stone-50 dark:bg-stone-900 border border-stone-300 dark:border-stone-700 rounded-xl flex flex-col"
            transition={{ type: 'spring', stiffness: 300, damping: 20 }}
        >
             <motion.div id="cursor" className="absolute z-20 pointer-events-none" style={{ opacity: 0, x: 150, y: -30 }}>
                <MouseCursorIcon className="w-5 h-5 text-stone-500 dark:text-stone-400 drop-shadow-lg" />
            </motion.div>
            <div className="flex-none px-3 py-2 border-b border-stone-200/50 dark:border-stone-800/50 flex items-center gap-6">
                ...
                <div className="flex items-center gap-1">
                    <button id="viz-tab-before" onClick={() => handleToggle('before')} className={`px-4 py-1.5 text-xs font-semibold rounded-md transition-colors ${isBefore ? 'bg-stone-200/80 dark:bg-stone-800/80 text-stone-900 dark:text-white' : 'text-stone-500 dark:text-stone-400 hover:bg-stone-200/50 dark:hover:bg-stone-800/50'}`}>
                        <div className="flex items-center gap-2">
                            <XIcon className="w-4 h-4 text-red-500" />
                            <span>Old Site</span>
                        </div>
                    </button>
                    <button id="viz-tab-after" onClick={() => handleToggle('after')} className={`px-4 py-1.5 text-xs font-semibold rounded-md transition-colors ${isAfter ? 'bg-stone-200/80 dark:bg-stone-800/80 text-stone-900 dark:text-white' : 'text-stone-500 dark:text-stone-400 hover:bg-stone-200/50 dark:hover:bg-stone-800/50'}`}>
                         <div className="flex items-center gap-2">
